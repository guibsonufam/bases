...
arq = csv.writer(open("eleicao.csv", "w"), delimiter='\t',quotechar='"')
count = 0
arq2 = open("eleicao.json", "w", encoding= 'utf-8')
arq3 = open("eleicao.txt", "w")
statuses = tweepy.Cursor(api.search, q=('#EleiçãoSemLulaÉFraude'), since='2018-01-22', until='2018-01-23').items()
row=[]
    #----------------------------------------------------------------#
...
while True:
    ...
    try:
         ...
         if(hasattr(status,"retweeted_status")):
            row = str(status.user.screen_name), str(status.user.id_str), str(status.user.followers_count), str(status.user.friends_count), \
                  ...
                  str(status.retweet_count), str(status.created_at), str(status.text)
    
         #str(status.retweeted_status.user.followers_count), str(status.retweeted_status.user.friends_count), \

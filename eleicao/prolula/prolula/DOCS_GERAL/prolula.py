...
arq = csv.writer(open("prolula.csv", "w"), delimiter='\t',quotechar='"')
count = 0
arq2 = open("prolula.json", "w", encoding= 'utf-8')
arq3 = open("prolula.txt", "w")
statuses = tweepy.Cursor(api.search, q=('#EleiçãoSemLulaÉFraude' or '#EleicaoSemLulaEFraude' or '#CadêAProva' or '#CadeAProva'), since='2018-01-24', until='2018-01-25').items()
row=[]
    #----------------------------------------------------------------#
    ...
    try:
         ...
         if(hasattr(status,"retweeted_status")):          
            row = str(status.created_at), str(status.retweeted_status.created_at), str(status.user.screen_name), str(status.user.id_str), \
                  ...
                  str(status.retweeted_status.user.screen_name), str(status.retweeted_status.user.id_str), str(status.retweeted_status.user.followers_count), \
